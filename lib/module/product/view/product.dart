...
class ProductView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    try {
            ...
            return BlocConsumer<VariantCubit, variant_states.VariantStates>(
              listener: (context, state) {
                if (state is variant_states.IneffectiveErrorState) {
                  AppSnackBar.error(context, state?.error ?? 'Unknown Error');
                }
              },
                  ...
                  body: DefaultBody(
                    child: Stack(
                      children: [
                        ...
                        ProductLoadingView(state: pState, vState: vState),
                      ],
                    ),
                  ),
                );
      ...
      log('Exception in ProductView : ${e.toString()}');
  ...
  Widget _body(BuildContext context, ProductViewStates state) {
    ...
    try {
      ...
      log('Exception in ProductView._body : ${e.toString()}');
      return _exceptionView(context, e.toString());
    }
  }
}
